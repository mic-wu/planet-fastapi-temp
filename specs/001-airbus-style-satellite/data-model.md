...
- `type` (enum: `image` | `video`): media type for rendering behavior
...
- `Story.type` maps to `FilterState.type` when filtering.
...
- Video stories must provide a playable source; otherwise fall back to image.
